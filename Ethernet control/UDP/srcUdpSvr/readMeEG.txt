...
This sample program is as same as the UDP server program written in [New Language Environment
 MOTO-PLUS Programmers Manual ].
FS100 side UDP server translates received strings which are sent by Windows PC client to large
 character and send back (Echo back) to the PC. 

Windows PC client application program is on a folder 
 C:\MotoPlusData\sample\Ethernet control\UDP\clientForWindows\client.exe
...
7) Insert "exit<CR>" the client is terminated. 
  ...
  ƒExecution example of DOS prompt„
	...
	C:\MotoPlusData\sample\Ethernet control\UDP\clientForWindows>client -----------<1>
	Simple UDP client
	Please type PORT No. >11000
	Please type IP addr. >192.168.255.1
	...
	C:\MotoPlusData\sample\Ethernet control\UDP\clientForWindows> -----------------<3>
